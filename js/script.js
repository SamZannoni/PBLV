...
function setup() {
  ...
  for (let i = 0; i < num_episode.length; i++) {
    ...
    if (num_episode[i] != num_episode[0]) {
      ...
      new_page.classList = "pageblanche";
      num_episode[i].parentNode.insertBefore(new_page, num_episode[i]);
      // console.log(num_episode[i].parentNode)
      // num_episode[i].appendChild(new_page);
    }
